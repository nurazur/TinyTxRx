...
// Version 7.1.2016 Umstellung auf ISO Week
...
// odd
// year change 2015 to 2016
//          Mo      Di      Mi      Do      Fr      Sa      So      Mo
//          28.12.  29.12.  30.12.  31.12.  1.1.    2.1.    3.1.    4.1.
// php      53      53      53      53      53      53      53      01
// python   52      52      52      52      00      00      00      01
...
function calc_python_week_from_time_stamp($day_time)
{
    $python_week = intval(date("W", $day_time));
    return $python_week;
}
...
function calc_logfilename($timestamp, $sensor = "bmp180")
{
    if ($sensor != "tinyrx")
    {
        $w = calc_python_week_from_time_stamp($timestamp);
        $c_month = intval(date("m", $timestamp));
        $y        = intval(date("Y", $timestamp));
        
        if($c_month ==  1 && $w >= 52) 
            $y = $y-1; // case 2015 / 2016
        else if($c_month == 12 && $w == 1)  
            $y = $y+1; // case 2014/2015
    ...
    {
        $logfile = calc_logfilename_tinyrx($timestamp);
    }    
    
    return $logfile;
}

function calc_logfilename_tinyrx($timestamp, $sensor = "tinyrx")
{
    $w = calc_python_week_from_time_stamp($timestamp);
    $c_month = intval(date("m", $timestamp));
    $y =       intval(date("Y", $timestamp));
    ...
    if ($w >9)
        $logfile = "../logfiles/".$y."_".$w;
    else
        $logfile = "../logfiles/".$y."_0".$w;
...
function generate_list_sensor($start_time, $stop_time, $sensor="bmp180")
{
    ...
    do
    {
        $w_new = calc_python_week_from_time_stamp($next_time);
        ...
        if (file_exists($logfilename))
        {
            if ($w_old != $w_new)
            {
                ...
                if ($lzi >0)
                {
                    ...
                    if ($sensor == "tinyrx")
                    {
                        $last_time_in_list = strtotime($zeile[1]." ".$zeile[2]);
                    }
                    else
                    {
                        $last_time_in_list = strtotime($zeile[2]." ".$zeile[3]); // $zeile[0] is the workweek, which is redundant because we know it from file name.
                    }
