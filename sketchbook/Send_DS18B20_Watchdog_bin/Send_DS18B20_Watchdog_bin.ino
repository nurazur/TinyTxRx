...
#define SENDDELAY  1584000  // wait this many ms between sending packets. 30 mins
//#define SENDDELAY  264000  // wait 5 mins
#define TXPOWER        5
...
//--------------------------------------------------------------------------------------------------
// forward encode a byte to an int
//--------------------------------------------------------------------------------------------------
uint16_t rf (unsigned char byte, char* codes)
{
    unsigned char hnibble = byte >>4;
    unsigned char lnibble = byte &0xf;
    return (codes[hnibble] << 8) | codes[lnibble];
}
...
void setup() {
  radio.Initialize(NODEID, FREQ, NETWORKID, TXPOWER); // Initialize RFM12 with settings defined above
  #ifdef KEY
...
void loop() {
  ...
  if (watchdog_counter >= watchdog_limit) {
    ...
    tinytx.temp = sensors.getTempCByIndex(0)*100; // read sensor 1
    ...
    #else
        /* encode FEC
        tinytx_fec.supplyV_h  = hamming_encode ((unsigned char)((tinytx.supplyV >> 8))&0xff,   hamming_codes);
        tinytx_fec.supplyV_l  = hamming_encode ((unsigned char)(tinytx.supplyV & 0xff), hamming_codes);
        //tinytx_fec.supplyV    = hamming_encode_24(tinytx.supplyV, hamming_codes);
        tinytx_fec.temp_h     = hamming_encode ((unsigned char)((tinytx.temp >> 8))&0xff,      hamming_codes);
        tinytx_fec.temp_l     = hamming_encode ((unsigned char)(tinytx.temp & 0xff),    hamming_codes);
        */ 
        hamming_encode_24 (tinytx.supplyV, tinytx_fec.vcc,  hamming_codes);     // 3 bytes with encoded FEC
        hamming_encode_24 (tinytx.temp/4+1000, tinytx_fec.temp, hamming_codes); // 3 bytes with encoded FEC

        // send
        radio.Send(GATEWAYID, &tinytx_fec, sizeof tinytx_fec, requestACK);
        //
        // if I want to send it without FEC, its like this:
