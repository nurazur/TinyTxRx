...
void RFM12B::Initialize(uint8_t ID, uint8_t freqBand, uint8_t networkid, uint8_t txPower, uint8_t airKbps, uint8_t lowVoltageThreshold)
{
  ...
  XFER(0x80C3 | (freqBand << 4)); // EL (ena TX), EF (ena RX FIFO), 12.0pF 
 ...
 // *** Receiver Control Command ***  
  //XFER(0x94A2);             // VDI,FAST,134kHz,LNA Gain 0dB,-91dBm 
  XFER(0x94C2);             // VDI,FAST,67kHz,0dB,-91dBm 
  
  //XFER(0x94CA);             // VDI,FAST,67kHz,LNA gain -6dB, DRRSI -91dBm 

  
 // *** Receiver Data Filter Command ***
  //XFER(0xC2AC);           // AL,!ml,DIG,DQD4
  //XFER(0xC2AD);           // AL,!ml,DIG,DQD5
  XFER(0xC2AF);             // AL,!ml,DIG,DQD7  
...
// ***FIFO and RESET Mode Command ***  
  ...
  XFER(0x9820 | (txPower > 7 ? 7 : txPower));   // !mp,45kHz,MAX OUT               //last byte=power level: 0=highest, 7=lowest
