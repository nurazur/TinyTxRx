...
class RFM12B
{
  ...
  void SPIInit();
    ...
    static volatile uint16_t status_reg;               // status register
    
    
    static void InterruptHandler();
    static uint16_t Control(uint16_t cmd);
    //Defaults: Group: 0xAA=170, transmit power: 0(max), KBPS: 38.3Kbps (air transmission baud - has to be same on all radios in same group)
  	void Initialize(uint8_t nodeid, uint8_t freqBand, uint8_t groupid=0xAA, uint8_t txPower=0, uint8_t airKbps=8, uint8_t lowVoltageThreshold=RF12_2v75);
    ...
    //void SendPrepare(uint8_t toNodeId, bool requestACK=false, bool sendACK=false);
    ...
    void SendPrbs9(uint8_t toNodeId, const void* sendBuf, uint8_t sendLen, bool requestACK=false, bool sendACK=false, uint8_t waitMode=SLEEP_MODE_PWR_DOWN);
